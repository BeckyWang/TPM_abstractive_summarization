...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, word2topic_encoder_states, topic_encoder_states, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  """
  Args:
    decoder_inputs: A list of 2D Tensors [batch_size x input_size].
    initial_state: 2D Tensor [batch_size x cell.state_size].
    encoder_states: 3D Tensor [batch_size x attn_length x attn_size(2*hidden_dim)].
    ...
    word2topic_encoder_states: 3D Tensor [batch_size x max_word2topic_enc_steps x 2*word2topic_hidden_dim].
    topic_enc_states: 3D Tensor [batch_size x max_topic_enc_steps x topic_hidden_dim].
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    #   v^T tanh(W_h h_i + W_WT h_WT + W_TW h_TW + W_s s_t + b_attn)
    # where h_i is an encoder state, h_WT is an word2topic encoder state , h_TW is an topic encoder stateand s_t a decoder state.
    # attn_vec_size is the length of the vectors v, b_attn, (W_h h_i) , (W_WT h_WT) , (W_TW h_TW) and (W_s s_t).
    ...
    if FLAGS.word2topic:
      # Reshape word2topic encoder_states (need to insert a dim)
      word2topic_attn_size = word2topic_encoder_states.get_shape()[2].value
      word2topic_encoder_states = tf.expand_dims(word2topic_encoder_states, axis=2) # now is shape (batch_size, attn_len, 1, word2topic_attn_size)   
      # Get the weight matrix W_WT and apply it to each word2topic encoder state to get (W_WT h_WT), the word2topic encoder features
      W_T = variable_scope.get_variable("W_WT", [1, 1, word2topic_attn_size, attention_vec_size])
      word2topic_encoder_features = nn_ops.conv2d(word2topic_encoder_states, W_T, [1, 1, 1, 1], "SAME") # shape (batch_size,attn_length,1,attention_vec_size)
    ...
    def attention(decoder_state, coverage=None):
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        _encoder_features = encoder_features
        if FLAGS.word2topic:
          _encoder_features += word2topic_encoder_features
        ...
        if use_coverage and coverage is not None: # non-first step of coverage
          ...
          e = math_ops.reduce_sum(v * math_ops.tanh(_encoder_features + decoder_features + coverage_features), [2, 3])  # shape (batch_size,attn_length)
          ...
          e = math_ops.reduce_sum(v * math_ops.tanh(_encoder_features + decoder_features), [2, 3]) # calculate e
    ...
    if FLAGS.topic:
      topic_vector = tf.squeeze(tf.reduce_sum(topic_encoder_states, axis=1))
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      if pointer_gen:
        with tf.variable_scope('calculate_pgen'):
          if FLAGS.topic:
            p_gen = linear([context_vector, topic_vector, state.c, state.h, x], 1, True) # a scalar
          else:
      ...
      # the shape of cell_output (batch_size, hidden_emb)
      # the shape of context_vector (batch_size, 2*hidden_emb)
