...
class Hypothesis(object):
  ...
  def length_norm(self):
    ...
    return math.pow(5 + token_len, FLAGS.length_norm_param) / math.pow(5 + 1, FLAGS.length_norm_param)

  @property
  def coverage_penalty(self):
    attns_of_source = np.sum(self.attn_dists, axis=0)
    attns_of_source = [math.log(min(a, 1.0)) for a in attns_of_source]
    return FLAGS.coverage_penalty_param * sum(attns_of_source)

  @property
  def new_log_prob(self):
    return self.log_prob / self.length_norm + self.coverage_penalty
...
def run_beam_search(sess, model, vocab, batch):
  ...
  enc_states, word2topic_enc_states, topic_enc_states, dec_in_state = model.run_encoder(sess, batch)
  # dec_in_state is a LSTMStateTuple
  # enc_states has shape [batch_size, <=max_enc_steps, 2*hidden_dim].
  # word2topic_enc_states has shape [batch_size, <=max_word2topic_enc_steps, 2*word2topic_hidden_dim].
  # topic_enc_states has shape [batch_size, <=max_topic_enc_steps, 2*topic_hidden_dim].
  ...
  while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
    ...
    (topk_ids, topk_log_probs, new_states, attn_dists, p_gens, new_coverage) = model.decode_onestep(sess=sess,
                        ...
                        word2topic_enc_states=word2topic_enc_states,
                        topic_enc_states=topic_enc_states,
  ...
  if FLAGS.use_coverage_penalty:
    hyps_sorted = new_sort_hyps(results)
  else:
...
def new_sort_hyps(hyps):
  """Return a list of Hypothesis objects, sorted by descending new log probability"""
  return sorted(hyps, key=lambda h: h.new_log_prob, reverse=True)
