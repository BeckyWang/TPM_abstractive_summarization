...
tf.app.flags.DEFINE_integer('word2topic_hidden_dim', 128, 'dimension of RNN hidden states(word2topic encoder)')
tf.app.flags.DEFINE_integer('word2topic_emb_dim', 65, 'dimension of word2topic distribution embeddings, it must be equal to the number of topic per text')
tf.app.flags.DEFINE_integer('topic_hidden_dim', 80, 'dimension of RNN hidden states(topic encoder)')
tf.app.flags.DEFINE_integer('topic_emb_dim', 30, 'dimension of topic distribution embeddings')
...
# Beam search
tf.app.flags.DEFINE_boolean('use_coverage_penalty', True, 'If true, use coverage penalty and length normarization in beam search.')
tf.app.flags.DEFINE_float('length_norm_param', 0.65, 'for length normarization in beam search')
tf.app.flags.DEFINE_float('coverage_penalty_param', 0.2, 'for coverage penalty in beam search')
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
  ...
  with sess_context_manager as sess:
    ...
    for i in range(0, iter_num):
      ...
      t1=time.time()
      tf.logging.info('iteration number: %i', i) # print the iteration number to screen
      ...
      if FLAGS.topic_aware:
        topic_loss = results['topic_loss']
        tf.logging.info("topic_loss: %f", topic_loss) # print the topic loss to screen

      # get the summaries and iteration number so we can write summaries to tensorboard
      summaries = results['summaries'] # we will write these summaries to tensorboard using summary_writer
      train_step = results['global_step'] # we need this to update our running average loss
      tf.logging.info('global step: %i', train_step) # print the global step to screen
...
def run_eval(model, batcher, vocab):
  ...
  # while True:
  for i in range(0, EVAL_ITERATION_PER_EPOCH):
...
def main(unused_argv):
  ...
  'hidden_dim', 'emb_dim', 
  'word2topic_hidden_dim', 'word2topic_emb_dim', 
  'topic_hidden_dim', 'topic_emb_dim', 
  'batch_size', 'max_dec_steps', 'max_enc_steps', 
  'coverage', 'cov_loss_wt', 'pointer_gen', 
  'topic_aware', 'topic_loss_wt'
  ]
  hps_dict = {}
